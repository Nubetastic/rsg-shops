...
-------------------------
-- setup ox_target zone
-------------------------
local function SetupZoneTarget(shopData)
    if not shopData or not shopData.shopcoords then return end

    local coords = shopData.npccoords or vector4(shopData.shopcoords.x, shopData.shopcoords.y, shopData.shopcoords.z, 0.0)
    local zoneCoords = vector3(coords.x, coords.y, coords.z)

    local success, err = pcall(exports.ox_target.addSphereZone, exports.ox_target, {
        coords = zoneCoords,
        radius = 1.5,
        debug = false,
        drawSprite = false,
